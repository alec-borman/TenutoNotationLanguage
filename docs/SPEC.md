...
**Implementation Languages:** Python 3.11+, JavaScript (Node.js 20+), WebAssembly  
...
Historically, digital music systems forced a strict dichotomy: either the **Helmholtz model** of static, discrete pitches (represented by XML sheet music), or the **Schaefferian model** of manipulatable sound objects (represented by raw `.wav` files and DAW project graphs). **Version 4.0.0 unifies these paradigms.** It acts as a universal logic layer capable of generating mathematically perfect Sheet Music (MusicXML), absolute performance data (MIDI 2.0), or rendering directly to audio via native DSP translation.

**Tenuto 4.0.0** is the canonical reference implementation for the **Declarative Protocol for Autonomous Engineering** (formerly NARE). The entire Tenuto ecosystem—compiler, engraver, runtime, and test harness—is written in **Python** and **JavaScript**, making it natively executable within Google AI Studio and other sandboxed LLM environments. This architectural choice transforms Tenuto into the ideal development use case for autonomous, AI‑driven software engineering.
...
4. **Electronic & Acoustic Parity:** Abstract DSP manipulations—such as time‑stretching, sidechain ducking, granular slicing, and 808 portamento glides—are elevated to semantic musical primitives, accessible via the same elegant, dot‑chained syntax used for classical articulations (`.stacc`).

### 1.2 The Compilation Pipeline (Tenuto 4.0.0)

A compliant Tenuto 4.0.0 compiler **MUST** implement a rigid 6‑stage transformation architecture. The canonical implementation is written in **Python** using the `lark‑parser` library for LL(1) parsing and the `fractions` module for rational arithmetic.
...
This is the minimum viable implementation required to be considered a Tenuto 4.0.0 Compiler.

- **Required Features:**
  - The Stateful Cursor (Sticky State).
...
### 1.5 Alignment with the Declarative Protocol for Autonomous Engineering

Tenuto 4.0.0 is engineered from the ground up to be the canonical demonstration of the **Declarative Protocol for Autonomous Engineering**. The entire toolchain—parser, engraver, test suite—is structured as a flat monorepo of `.py` and `.js` files, enabling the Protocol’s Unbound Implementer to execute deterministic file mutations and autonomous verification using `pytest` and `node --test`.

- **Tela Task (YAML TDB):** Every feature addition or bug fix to the Tenuto codebase is specified as an atomic YAML Test‑Driven Boundary. The Actuator receives the TDB, edits the relevant source files, and autonomously runs the verification suite to prove correctness.
- **Verification Block:** The Actuator outputs a `<verification>` block containing the raw `pytest` or `node --test` output, providing cryptographic proof that the mutation satisfied the constraints.
- **Master Builder’s Oath:** The human developer writes zero implementation code during active sprints. They define the Intent, provide the Canonical Snapshot, and merge verified patches.

**Implementation Language Migration Summary:**

| Component                  | Tenuto 3.0.0 (Rust)          | Tenuto 4.0.0 (Python/JS)                     |
|----------------------------|------------------------------|----------------------------------------------|
| Compiler (`tenutoc`)       | Rust → WASM                  | Python (`lark`, `fractions`) → Pyodide/WASM  |
| Engraver (`tenuto‑engrave`)| Rust (`salsa`, `kurbo`)      | Python (`kiwisolver`, `bezier`, `numpy`)     |
| Runtime (`tenutod`)        | Rust binary                  | Node.js + Python microservices               |
| Test Suite                 | `cargo test`                 | `pytest` + `node --test`                     |
| Sandbox Environment        | N/A                          | Google AI Studio (native Python/Node)        |
...
|------|------------------------|---------------------|
| **Integer** | `[0-9]+` | Parsed as `int`. |
| **Float** | `[0-9]+\.[0-9]+` | Parsed as `float`. |
| **StringLit** | `"(?:[^"\\]|\\.)*"` | Standard quoted string. |
...
| **TimeVal** | `[0-9]+(?:\.[0-9]+)?(?:ms\|s\|ticks)` | Absolute physical time units. Used in `style=synth` envelopes and micro‑timing offsets. |
...
- **Compiler Fallback Rule (Graceful Degradation):** If a v4.0 compiler encounters `tenuto "3.0"` or `tenuto "2.2"`, it **MUST** load the corresponding legacy parsing matrix. This ensures that older syntax versions execute perfectly, while safely ignoring or warning against unmapped v4.0 features.
...
| `swing` | Integer / Map | Applies a localized micro‑timing algorithm. An Integer (e.g., `66`) applies a global swing percentage to all subdivisions. A Map enables grid‑specific swing depths, e.g., `@{ 8: 66, 16: 54 }` applies a heavy 66% swing to eighth notes, but a lighter 54% swing to sixteenth notes. |
| `sidechain` | Map | Establishes global signal ducking routes (Detailed in Section 12). |
...
4. **`style=concrete`:** The Schaefferian Model. Bypasses MIDI synthesis entirely. Maps alphanumeric keys to absolute physical timestamps of an external binary `.wav` file, evaluating stretch, slice, and granular algorithms in the audio backend.
5. **`style=synth`:** The Continuous Frequency Model. Treats pitch not as discrete events, but as a continuous frequency function. Configures global ADSR envelopes, LFOs, and portamento glide limits.
...
- **The Mathematics of Drift Prevention:** Floating‑point representations of time (e.g., $0.333333$) compound inaccuracies over thousands of events. The Tenuto compiler stores logical duration strictly as a `Fraction` (Numerator / Denominator). A quarter note is mathematically $1/4$. A triplet eighth note is calculated algebraically: $1/8 \times 2/3 = 1/12$.
- **Late Resolution:** The IR only coerces these fractions into absolute scalar ticks (`int`) at the exact moment of calculating the `AtomicEvent`, guaranteeing $0.00\%$ temporal drift across symphonies of any length.
...
### 5.3 Micro‑Timing & “The Pocket” (Physical Time)
...
Tenuto 4.0.0 introduces Micro‑Timing modifiers. These alter the **Physical Playback Time** (the absolute audio gate tick where the trigger fires) but leave the **Logical Grid Time** mathematically perfect.
...
- **Cursor Immunity:** Grace duration is **NOT sticky**. The state cursor retains the last “Real” rational duration, ensuring the note following a grace note inherits the correct structural rhythm.
...
- The compiler logarithmically derives all other pitches relative to this physical constant, tracking internal frequency states via `float` precision before casting to integer MIDI boundaries (`69`).
...
Modifiers are commutative regarding their semantic execution (the order of chaining does not affect the mathematical output), but the parser processes them sequentially into an `attributes: list[Attribute]` array. The compiler categorizes attributes to determine exactly how they alter the IR.
...
$$Pitch = Tuning[len(Tuning) - String] + Fret + CapoOffset$$
...
## 10. The Concrete Engine (Sampling)

Modern production heavily relies on the manipulation of raw audio waveforms (sampling). Standard DAWs manage this via opaque GUI stretch‑markers and massive binary files. Tenuto 4.0.0 abstracts this into a purely semantic, token‑efficient logic layer. *Note: Tenuto does not embed binary audio; it emits structural DSP instructions to the rendering backend.*
...
## 11. The Synth Engine (Physics & ADSR)
...
## 12. Dynamic Signal Interaction (Sidechaining)

“Pumping” or ducking a signal (e.g., rapidly attenuating the volume of an 808 every time the kick drum hits) is mandatory in modern production. Tenuto 4.0.0 provides two mathematically deterministic methods for sidechain ducking, nullifying the need for convoluted DAW routing graphs.
...
Tenuto supports independent rhythmic streams within a single staff, essential for piano scores, drum sets, and complex electronic automation. Version 4.0.0 supercharges the polyphonic engine by bifurcating the Tuplet syntax to natively support Euclidean rhythm generation algorithms.
...
### 13.2 Disambiguation: Euclidean vs. Polyrhythmic Tuplets

Modern electronic beats (e.g., trap, reggaeton, afrobeat) rely heavily on algorithmic, off‑grid syncopation—distributing $K$ hits as evenly as possible across $N$ subdivisions. Tenuto 4.0.0 natively executes the **Euclidean Algorithm** $E(K, N)$ through a highly token‑efficient overloading of the Tuplet syntax.

To prevent backwards‑compatibility breakage with Tenuto v3.0, the deterministic LL(1) parser evaluates the *internal content* of the parentheses to route to the correct mathematical solver.
...
Tenuto transitions abstract musical intent into dense physical control streams. While standard dynamics (`.ff`, `.p`) handle static velocity, Tenuto 4.0.0 introduces high‑resolution interpolation for continuous control across the absolute timeline.
...
- **`.tr` (Trill) / `.mord` (Mordent) / `.turn`:**
  - *Visual:* Injects standard SMuFL ornamentation glyphs (e.g., `ornamentTrill`) above the staff’s Top Skyline.
...
- **`.gliss` (Glissando) / `.port` (Portamento):**
  - *Visual:* Instructs the `kurbo` subsystem to route a continuous line or Bezier curve between the source note’s $P_0$ anchor and the target note’s $P_3$ anchor.
...
Tenuto 4.0.0 supports Just Intonation, Xenharmonic scales, and global non‑12TET mapping natively in both visual formatting and audio synthesis.
...
- **`E1004: Version Incompatible`** – Target parser is older than the required `tenuto "4.0"` version declaration.
...
2. **Rational Arithmetic Requirement:** Floating point numbers (`float`) **MUST NOT** be used to calculate temporal logic (durations, tuplets). Subdivision calculations must utilize explicit `Fraction` objects until final integer tick resolution.
...
This section provides the **Normative Syntax** of Tenuto v4.0 using Extended Backus‑Naur Form (EBNF). It encompasses the v4.0 compound sigils, the `TimeVal` physical time parameters, and the bifurcated Euclidean tuplet syntaxes.
...
## 31. Reference Example (The v4.0 Producer Suite)

This comprehensive file serves as the ultimate validation target for the Tenuto 4.0 compiler, unifying Euclidean sampling, action‑notation sidechain ducking, ADSR synthesizer physics, and advanced acoustic notation into a single, cohesive semantic matrix.
...
**Version:** 1.0 (Extension to Tenuto 4.0.0)  
**Status:** Normative / Final  
...
The Tenuto 4.0 language natively resolves the “Semantic Gap” between acoustic composition and modern electronic production. However, to maintain maximum token efficiency for AI generation and human readability, the Tenuto compiler (`tenutoc`) **MUST NOT** attempt to internally replicate the low‑level digital signal processing (DSP) engines of highly specialized audio languages.
...
*Tenuto 4.0.0 Implementation Note:* Delegation backends are implemented as Python subprocess wrappers, allowing the Actuator within the Declarative Protocol for Autonomous Engineering to verify correct OSC packet generation using deterministic mock servers.
...
| Tenuto 4.0 Syntax | SuperDirt OSC Parameter | Compiler Execution Behavior |
|-------------------|-------------------------|-----------------------------|
...
*Declarative Protocol Integration:* The `verification` block of a Tela Task can execute headless Chrome with Puppeteer to confirm that the WASM module produces the correct AudioWorklet graph.

## B.2 The WebAssembly Compiler Target (`--target wasm`)

The core Python `tenutoc` compiler **MUST** support a `wasm32‑unknown‑unknown` build target via **Pyodide** (Python compiled to WebAssembly). This allows the entire lexing, parsing, and IR unrolling process to execute client‑side within a standard JavaScript/TypeScript environment.
...
In Tenuto 4.0, chaining `.ped` to pitch events bloated the AST and made rhythm edits destructive to the pedaling logic. Addendum C introduces decoupled control lanes.
...
The “Sticky State” architecture of Tenuto 4.0 assumes absolute octave retention. If a user writes `b4 c`, the compiler assumes `c4`, resulting in a massive downward leap of a Major 7th. To prevent AI context drift over long horizontal arpeggios, Tenuto introduces the Relative Pitch heuristic.
...
The engine operates entirely offline within the Python compiler. It actively hunts for mathematical patterns, physical instrument constraints, and structural redundancies, refactoring them into Tenuto’s highest‑level abstractions (Macros, Euclidean tuplets, Stateful Cursors, and Decoupled Control Lanes) without utilizing non‑deterministic AI generation.

*Implementation:* The decompiler is written in pure Python using `music21` for MusicXML parsing, fully verifiable within the Declarative Protocol sandbox.
...
Tenuto 4.0 unifies the discrete (sheet music) and the continuous (DSP/audio) into a single Abstract Syntax Tree (AST). However, when a compiler is executed with a discrete visual target (e.g., `tenutoc --target musicxml` or `--target svg`), it encounters **Unprintable Physics**—events and modifiers that possess absolute temporal geometry but zero diatonic visual spelling (e.g., a `.wav` file sliced via `style=concrete`, or an invisible `.cc` sidechain curve).
...
- **Behavior:** The compiler intercepts the active REPL memory, materializes the hidden Auto‑Scaffolding wrapper, extracts the accumulated continuous timeline, and outputs a fully formed, valid `tenuto "4.0"` source file.
...
**Version:** 1.0 (Extension to Tenuto 4.0.1)  
**Status:** Normative / Final  
...
**Addendum G** collapses the mixing console into the Tenuto 4.0 Abstract Syntax Tree (AST). By strictly adhering to the **“Narrow Waist” Layered Architecture**, Tenuto serializes post‑production intent as abstract mathematical data in the Intermediate Representation (IR). It stores the *concept* of an acoustic space, guaranteeing that a cinematic, heavily‑processed track will render perfectly a century from now, regardless of the underlying physical audio hardware.
...
tenuto "4.0" {
  ...
  measure 80 {
    ...
    sub: eb1:1.glide(4s) eb0:1 |

    glitch: capture:1.slice(16).reverse.pan([-1.0, 1.0], "linear") |

    master: s:1.fx(reverb, @{ mix: [0.0, 1.0], decay: 8s }) |
  }
}
```
...
**Version:** 1.0 (Integration with Tenuto 4.0.1)  
**Status:** Normative / Final  
...
This addendum is considered **Final**. The `tenutod` Python implementation shall follow the **Asynchronous Packet Pacing** model, ensuring that network hardware buffers are filled exactly **50ms** ahead of the gPTP execution deadline.
...
As a Tenuto 4.0 implementation matures toward a “System of Record” for music, the underlying codebase (including the Python compiler, Wasm runtime, and WebGL/WebAudio engines) inevitably expands beyond the reliable “lost‑in‑the‑middle” thresholds of standard LLM context windows. To maintain 10x development velocity at a scale of **50,000+ lines of code**, the environment **MUST** move from raw file‑dumping to a **Domain‑Aware Retrieval‑Augmented Generation (RAG)** system.
...
The indexer **SHOULD** utilize a **Tree‑sitter** parser for Python and TypeScript (via the Python `tree_sitter` binding) to identify the fundamental logical boundaries of the code.

- **The Rule of Atomic Logic:** A “Chunk” **MUST** represent a complete semantic unit—such as a single `class`, a top‑level `function`, or a module‑level constant block.
...
To prevent “Domain Contamination” (where the AI confuses WebGL shader math with Python AST logic), the indexer **MUST** apply a multi‑dimensional metadata matrix to every vector.
...
| `criticality` | `high` \| `medium` \| `low` | Prioritizes “Narrow Waist” logic (e.g., `ir.py`) over UI components. |
| `language` | `python` \| `javascript` \| `tenuto` | Ensures syntax‑specific retrieval. |
...
*Tenuto 4.0.0 Note:* The DAW is implemented as a **Vite + React** application that communicates with a Python WebSocket server running the compiler. This architecture is fully compatible with the Declarative Protocol’s file‑based actuation model.
...
# Addendum K: The Skybridge Protocol (Teleportation Ecosystem Integration)

**Version:** 1.0 (Extension to Tenuto 4.0.1)  
**Status:** Normative / Final  
**Scope:** Open‑Source Transparency, The Tandem Skyscraper Architecture, and Geometric Governance.

## K.1 Architectural Transparency: Open‑Source vs. Internal Infrastructure

Tenuto 4.0 is a fully open‑source, standalone digital signal processing and musical logic language. However, developers and community contributors reviewing the core Tenuto codebase may encounter references to the `.tela` format, 1024‑dimension “intent vectors,” and the “Teleportation Execution Engine.”

To prevent community confusion, this addendum formally clarifies the relationship between Tenuto and the Teleportation Protocol:

- **Tenuto 4.0 (Open Source):** The public‑facing, flagship audio and execution engine. It is fully functional, sovereign, and requires absolutely no proprietary tools to compile, run, or render music.
- **The .tela Teleportation Protocol (Internal/Unreleased):** The proprietary, meta‑architectural physics engine used by the core maintainers to *author* the Tenuto 4.0 compiler. It uses deterministic, 1024‑dimensional vector geometry to leash AI developers and mathematically guarantee the structural integrity of the Tenuto codebase.

## K.2 The Tandem Skyscrapers

Tenuto and the Teleportation Protocol are being developed simultaneously as two distinct, sovereign structures—a paradigm we call the **Tandem Skyscraper Architecture**.

- **The Tenuto Skyscraper:** Handles the specific domain logic of acoustic physics, rational timelines, and WebAssembly audio execution.
- **The Tela Skyscraper:** Handles the meta‑architectural physics, governing determinism, intent preservation, and the strict LL(1) Narrow Waist grammar.

Tenuto serves as the definitive production “Day Zero” use‑case for the Teleportation Protocol. It acts as the ultimate proving ground, ensuring that the `.tela` vector geometry is forged under the extreme pressure of real‑world digital signal processing constraints.

## K.3 The Skybridge (One‑Way Dependency)
...
- **Core Development:** When the core team architects a new feature for Tenuto (e.g., Euclidean tuplets or micro‑timing), it is first mathematically defined in the closed‑source `.tela` Manifest. The AI generates the Python implementation by reducing the Vector Delta ($\Delta$) to 0.00.
- **End‑User Execution:** Once the Python code is poured and committed to the Tenuto repository, the Skybridge is severed for the end‑user. The Tenuto community does *not* need the `telac` Embedding Compiler to parse Tenuto files, generate MIDI, or run the WebAudio runtime. The open‑source Tenuto binary is a fully sovereign artifact.

## K.4 Community Contributions vs. Core Governance

Because the Teleportation Execution Engine strictly governs the Tenuto architecture, open‑source community contributions are seamlessly integrated through the Skybridge:

- **Community PRs:** Open‑source contributors can write standard Python and JavaScript pull requests for Tenuto without ever learning or touching a `.tela` file.
- **Vectorized Audits:** When a community PR is submitted, the core maintainers will internally pass the proposed code through the `.tela` Embedding Compiler (`telac`). The system will perform a topological traversal of the Abstract Syntax Tree (AST) to generate a 1024‑dimension geometric coordinate for the PR. If the community code aligns with the mathematical gravity of the Tenuto architecture, it is merged. If it introduces an architectural regression (a massive Vector Delta spike), the core team will refactor it.

*Tenuto 4.0.0 Note:* The `telac` audit tool is a Python script that computes the Vector Delta between two AST snapshots, exactly as described in the **Declarative Protocol for Autonomous Engineering**’s Gauntlet specification.

## K.5 The Decoupling Mandate (Standalone Sovereignty)

While Tenuto acts as the crucible for Tela’s genesis phase, the `.tela` protocol is fundamentally
